...
export async function PATCH(request: Request) {
  try {
    ...
    let body: Parameters<typeof parseBrandForm>[0] & { brandId?: string; id?: string }
    try {
      body = (await request.json()) as typeof body
    } catch {
    ...
    const result = await updateDb((next) => {
      const current = next.users.find((row) => row.id === auth.user.id)
      if (!current) return null
      const updated = updateAssignedBrandProfile(current, brandId, located)
      if (!updated) {
        const error = new Error("Brand not found on this account.") as Error & { status?: number }
        error.status = 404
        throw error
      }
      return {
        brand: brandQuotaView(updated),
        brands: current.aiBrands.map(brandQuotaView),
      }
    })
