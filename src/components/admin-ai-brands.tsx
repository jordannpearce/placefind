    ...
    <section className="space-y-5 rounded-2xl border bg-card p-5">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          ...
          agency. Prompt scans on that workspace check the company name, address, phone, and website.
        </p>
      </div>
      ...
      <form onSubmit={(event) => void submit(event)} className="grid gap-3 md:grid-cols-2">
        ...
        <div className="space-y-1.5 md:col-span-2">
          <Label htmlFor="admin-brand-address">Address</Label>
          <Input
            id="admin-brand-address"
            value={form.address}
                ...
                <tr key={`${brand.ownerUserId}-${brand.id}`} className="border-t">
                  <td className="px-3 py-3">
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      {[brand.website || brand.domain, brand.phone].filter(Boolean).join(" · ") || "No site"}
                    </p>
                  </td>
                  <td className="px-3 py-3">
