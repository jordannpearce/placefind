...
describe("public website copy", () => {
  ...
  it("keeps Google Maps off the homepage and prices a listing at $150 per month", () => {
    ...
    assert.match(directory, /joinHref\("member"\)/)
    assert.match(directory, /Join to leave reviews/)
    assert.match(directory, /Create a Profile/)
  ...
  it("embeds the Preferred Sources button on the homepage and site footer", () => {
    ...
    assert.equal(/Prefer PlaceFind in Google Search/.test(home), false)
    assert.equal(/Add PlaceFind as a preferred source/.test(footer), false)
